 ...
 *
 * 執行方式：node scripts/convert-lessons.cjs
 */
...
English:`;
  ...
  try {
    ...
    console.error(`  ✗ 翻譯失敗 (中→英): ${chineseText}`, error.message);
    throw error;
  }
}
...
Traditional Chinese:`;
  ...
  try {
    ...
    console.error(`  ✗ 翻譯失敗 (英→中): ${englishText}`, error.message);
    throw error;
  }
}
...
async function convertStep(step, stepIndex) {
  ...
  await delay(7000); // 7秒延遲避免 rate limit
