  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4">
        ...
        <p className="text-sm text-gray-600">與 AI 教師即時對話</p>
      </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-6">
          ...
          {messages.length === 0 && suggestions.length === 0 && (
            <div className="flex h-full items-center justify-center">
              <div className="text-center text-gray-400">
                ...
                <p className="text-sm">對話內容將顯示在這裡</p>
              </div>
            </div>
          )}
          ...
          {messages.map((message) => (
            ...
            >
              ...
              >
                {/* English Text (Primary Learning Language) */}
                <div className="text-base font-medium">{message.english}</div>

                {/* Chinese Translation (for instructor messages) */}
                {message.role === 'instructor' && message.chinese && (
                  <div className="mt-1 text-sm text-gray-600 italic">{message.chinese}</div>
                )}
